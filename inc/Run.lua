...
kalia = Token:match("(%d+)")
redis:set(kalia..":VERSION",1)
redis:set(kalia..":SUDO_ID:",SUDO_USER)
redis:set(kalia..":DataCenter:",'German')
redis:set(kalia..":UserNameBot:",BOT_User)
redis:set(kalia..":NameBot:",BOT_NAME)
redis:hset(kalia..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3kalia_INSTALL","Yes")
info = {} 
...
print('\27[0;33m>>'..[[
echo -e "\e[38;5;77m       •{ تنصيب سورس كــآلــيـــآ بنسبه 100% ]  }•\e[0m"
echo -e "\e[38;5;77m       •{ •{ Join the Source Channel : @UU_YEV }• }•\e[0m"
...
echo -e "\e[38;5;77m       •{ SOURCE KALIA source has been installed }•\e[0m"
echo -e "\e[38;5;77m       •{ developer JACK : @PHP57  }•\e[0m"

echo -e "\e[38;5;77m      KALIA ✾ @PHP57e[0m"
...
kalia = Token:match("(%d+)")
our_id = tonumber(kalia)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(kalia..":UserNameBot:")
SUDO_ID = tonumber(redis:get(kalia..":SUDO_ID:"))
SUDO_USER = redis:hgetall(kalia..'username:'..SUDO_ID).username
version = redis:get(kalia..":VERSION")
DataCenter = redis:get(kalia..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..kalia..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(kalia..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(kalia..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(kalia..'group:add'..msg.chat_id_) then 
...
elseif msg.sender_user_id_ == 1681170979 then 
msg.TheRank =  '  مطوره السورس '
msg.Rank = 1
...
msg.Rank = 1
elseif redis:sismember(kalia..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(kalia..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(kalia..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(kalia..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(kalia..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(kalia..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(kalia..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(kalia..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(kalia..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(kalia..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(kalia..':WELCOME_BOT'),[[• مرحباً انا بوت ]]..redis:get(kalia..':NameBot:')..[[ .
• اختـصاصـي حمايـة الـمجموعات
• تأكد من رفعـك للبـوت مشـرف في مجموعـتك واكتب تفعـيل 
• قنـاة سورس كــآلــيـــآ ⇐ @UU_YEV
...
if msg.GroupActive and redis:get(kalia..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(kalia..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(kalia..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(kalia..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.kalia then 
if msg.text and ScriptFile.ikalia then
for k, kalia in pairs(ScriptFile.kalia) do
local SearchText = msg.text:match(kalia)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.ikalia(msg,{msg.text:match(kalia)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",kalia.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dkalia then
if ScriptFile.dkalia(msg) == false then
return false
end
...
if Plug.kalia then 
if msg.text and Plug.ikalia then
for k, kalia in pairs(Plug.kalia) do
local SearchText = msg.text:match(kalia)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(kalia..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(kalia..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.ikalia(msg,{msg.text:match(kalia)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",kalia.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source kalia")
end 

end
else
print("The File Script.lua Not Runing in The Source kalia")
...
function tdcli_update_callback(data)
	...
	redis:srem(kalia..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/jack1eljoker1/KALIA/main/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'SOURC')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/KALIA/main/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'SOURCE')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/KALIA/main/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'SOURCE KAL')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/KALIA/main/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/KALIA/main/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/KALIA/main/plugins/games.lua','./plugins/games.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'SOURCE KALIA\n\n• ** السورس الى اصدار \n• ** تم اعاده تشغيل سورس كــآلــيـــآ بنجاح')
	dofile("./inc/Run.lua")
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(kalia..":SUDO_ID:")
redis:hset(kalia..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(kalia..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(kalia..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(kalia..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(kalia..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(kalia..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(kalia..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(kalia..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(kalia..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(kalia..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(kalia..'group:add'..data.chat_.id_) then
	redis:set(kalia..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(kalia..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(kalia..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(kalia..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	kalia..'group:name'..k,IDS.Title,
	kalia..'num_msg_kalia'..k,5,
	kalia..'group:add'..k,true,
	kalia..'lock_link'..k,true,
	kalia..'lock_spam'..k,true,
	kalia..'lock_webpage'..k,true,
	kalia..'lock_markdown'..k,true,
	kalia..'lock_flood'..k,true,
	kalia..'lock_bots'..k,true,
	kalia..'mute_forward'..k,true,
	kalia..'mute_contact'..k,true,
	kalia..'mute_document'..k,true,
	kalia..'mute_inline'..k,true,
	kalia..'lock_username'..k,true,
	kalia..'replay'..k,true
	)
	redis:sadd(kalia..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(kalia..'username:'..ID,'username',user)
	redis:sadd(kalia..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(kalia..'username:'..ID,'username',user)
	redis:sadd(kalia..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(kalia..'username:'..ID,'username',user)
	redis:sadd(kalia..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'• ** تم رفع النسخه‏‏ الاحتياطيه\n• ** حالياً عدد مجموعاتك هيه‏ *'..redis:scard(kalia..'group:ids')..' .*\n')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(kalia..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(kalia..'username:'..data.user_.id_).username
	...
	redis:hset(kalia..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
